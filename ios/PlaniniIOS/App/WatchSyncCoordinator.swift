...
private let watchSyncLog = Logger(
    subsystem: "de.malaber.planini.ios",
    category: "watch-sync"
)
...
final class WatchSyncCoordinator: NSObject {
    ...
    func publishCurrentState() {
        ...
        try? session.updateApplicationContext([PlaniniSharedConstants.watchContextPayloadKey: data])
    }
}
...
extension WatchSyncCoordinator: WCSessionDelegate {
    ...
    func session(
        ...
        replyHandler([PlaniniSharedConstants.watchContextPayloadKey: data])
    }
}
#else
